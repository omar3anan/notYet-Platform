...
function ExerciseeThree() {
    ...
    const fetchData = async () => {
      try {
        ...
        const filteredData = responseData.data.filter(
          (problem) =>
            (problem.difficulty === "Hard" && problem.category === "Stack") ||
            (problem.category === "LinkedList" &&
              problem.difficulty === "Hard") ||
            (problem.category === "Greedy" && problem.difficulty === "Hard")
        );
  ...
  return (
    <div>
      ...
      <div>
        <Backgroundgif />
        <h1 className="Text">DATA Strcucture LEVEL 3</h1>
