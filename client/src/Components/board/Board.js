...
class Board extends React.Component {
  ...
  fetchSketchDataFromDB() {
    ...
    fetch(`http://localhost:5000/whiteboard/${sessionId}`)
      .then((response) => response.json())
      .then((sketchData) => {
        this.setState({ sketchData });
        const currentSketchId = sketchData._id;
        console.log("SketchData from DB: ", sketchData);
      })
      .catch((error) => {
  ...
  saveSketchDataToDB = () => {
    ...
    fetch("http://localhost:5000/drawing/", {
      ...
      .then((response) => response.json())
      .then((data) => {
        console.log("Sketch data saved: ", data);
      })
      .catch((error) => {
        console.log("Error saving sketch data:", error);
      });
  };
